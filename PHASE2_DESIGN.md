# PHASE2_DESIGN.md — dual-ai-review

Design document for Phase 2A (fully automatic ChatGPT ↔ Claude review loop). This is a
design only — no implementation code is included. It builds directly on the accepted,
frozen Phase 1 foundation (see `REVIEW.md`) and follows the extension points already
reserved for it in `SPEC.md` §15 and `DECISIONS.md` §6.

**Constraints carried over from the user's Phase 2 brief (binding on every section below):**
- Web UI automation only — no Codex CLI, no Claude Code CLI, no OpenAI/Anthropic API, at
  runtime.
- No manual copy/paste between ChatGPT and Claude — the program does the handoff.
...
- No Cloudflare bypass, no stealth plugins, no fingerprint spoofing — ever.
...
reviewMain.ts (new composition root)
  ...
  ├─ discoverSitePages()              [unchanged]
  ├─ chatgptSite.ts / claudeSite.ts   [extended: + sendPrompt/wait*/getLatest*]
  ├─ orchestrator/reviewOrchestrator.ts  [new] — depends only on the adapter interface
  ├─ orchestrator/convergence.ts         [new] — pure text → {status, summary, issues}
  ├─ orchestrator/prompts.ts             [new] — pure template functions
  └─ orchestrator/runStore.ts            [new] — runs/<id>/ persistence + resume
```

`ReviewOrchestrator` never imports `chatgptSite.ts`/`claudeSite.ts` directly — it is
constructed with two objects satisfying a shared adapter interface, exactly as
`DECISIONS.md` §6 requires. This is what makes it unit-testable with fake, in-memory
adapters and zero browser dependency (see §11).
...
alongside them, per `SPEC.md` §15's designated extension point).
...
Where `baseline` is a `TurnBaseline` captured immediately **before** `sendPrompt` (see §10)
— every method after `sendPrompt` is parameterized by it, so "new" is always defined
relative to a concrete snapshot, never "whatever looks newest right now."

### `sendPrompt(page, text)`
1. Locate the composer via the ChatGPT selector fallback group (§9). Fail (throw a typed
   `ComposerNotFoundError`) if none match — this is a distinct, orchestrator-visible error,
   not a silent no-op.
...
3. Submit via the send-button selector fallback group (§9), not the Enter key by default —
   a dedicated button click is unambiguous regardless of the composer's Enter-vs-Shift+Enter
...
4. Do not wait for anything here — waiting is the caller's job via the next two methods,
...
- the stop/streaming-cancel button (selector fallback group, §9) becomes visible, or
- the assistant-message count (re-queried) exceeds `baseline.count`.

If neither happens within the timeout, return `"not_observed"` rather than throwing —
some responses are fast enough that generation completes before this check would ever
...
Two-signal design — never a fixed sleep:
...
2. **Confirmation (stability) signal:** once (1) is true, read the candidate new message's
   ...
   scratch. This combination — UI state first, text stability as confirmation — avoids both
...
3. Bounded by `timeoutMs` (config: `generationTimeoutMs.chatgpt`, default `180000`). On
   timeout, return `{ outcome: "timeout", partialText }` rather than throwing — the caller
   (orchestrator) decides what "timeout" means for the run (§8), and the partial text is
   preserved for diagnosis (saved alongside a screenshot, reusing `openSite.ts`'s
   `saveScreenshot` helper).
4. Also watches for an explicit error/regenerate banner (selector fallback group, §9,
   ...
   timeout — this is a faster, more specific failure than a generic timeout.
...
`baseline`, using the id-diff strategy in §10. Throws a typed
`AmbiguousNewMessageError` if zero or more-than-one new message is found — the
orchestrator treats this the same as a malformed/empty response (§8), pausing for manual
...
stop/streaming-indicator details differ (§9) — the send/wait/extract *logic* described
...
(selectors, what "streaming" looks like) stays site-specific, per `DECISIONS.md` §6.

Claude-specific notes:
- Claude's composer and message list use different attributes than ChatGPT's; the fallback
  groups in §9 encode this, nothing else differs architecturally.
- Claude's equivalent of ChatGPT's "regenerate/error banner" (e.g. a Cloudflare
  ...
  fallback group, returned as `error_banner`, never auto-dismissed or clicked through — if
  ...
  (already reused as a pre-flight guard, see §8) is what ultimately catches it, since a
...
```
It depends only on `ConversationalSiteAdapter` (§13) — never on `chatgptSite.ts`/
`claudeSite.ts` concretely — so `tests/unit/orchestrator.spec.ts` can drive it with two
fake, in-memory adapters and assert its full state machine with no browser at all (§11).
...
- Before every send (ChatGPT or Claude), re-runs that site's existing `checkReady(page)`
  as a pre-flight guard (reusing Phase 1's `SiteLoadStatus` classification verbatim) — if
  not `"ready"`, the run pauses per §8 instead of sending into a login/challenge/unknown
  page.
- Drives the ChatGPT → Claude → (loop) handoff described in §4.
- Calls `runStore` after every completed turn and every status change (§7) — persistence
  is not a bolt-on at the end, it is how the orchestrator survives a crash mid-run.
- Owns the single retry-on-malformed-convergence attempt (§5) — no other retries exist
...
- Exposes `run(): Promise<RunResult>` for a fresh run and `resume(runId): Promise<RunResult>`
  for continuing a persisted one (§7).
- Extension point for Phase 2B (§12): accepts an optional
...
**Round 1**
1. Build the initial ChatGPT prompt from `task` (template in §6).
2. `chatgpt.sendPrompt` → `waitForGenerationStart` → `waitForGenerationComplete` →
   `getLatestAssistantResponse` → this is `round-01-chatgpt`.
3. Build the Claude reviewer prompt from `task` + `round-01-chatgpt` text (template in §6).
4. `claude.sendPrompt` → wait → `getLatestAssistantResponse` → this is `round-01-claude`.
5. Parse `round-01-claude` with `convergence.ts` (§5).
6. If `CONVERGED` → run status `converged`, write `final.md`, stop.
7. If `CONTINUE` → proceed to Round 2.
...
1. Build the ChatGPT revision prompt from the **issues list** parsed out of the previous
   ...
   full — see §6 on why). Both ChatGPT's and Claude's tabs already hold the full
...
2. `chatgpt.sendPrompt` (same tab/conversation) → wait → `getLatestAssistantResponse` →
   `round-0N-chatgpt`.
3. Build the subsequent Claude review prompt referencing "the revised answer" (§6).
4. `claude.sendPrompt` (same tab/conversation) → wait → `getLatestAssistantResponse` →
   `round-0N-claude`.
5. Parse; `CONVERGED` → stop. `CONTINUE` → increment round, repeat.
6. If `N == maxRounds` after step 5 and status is still `CONTINUE` → run status
...
`stalled_no_progress` pause instead of burning the remaining rounds — this is a literal
string-equality check, not natural-language similarity, so it doesn't conflict with "do not
depend only on NL similarity" (§5); it only catches the degenerate case of Claude repeating
...
Parsing rules (deterministic, tested against many fixtures — §11):
1. Find every line matching `/^\s*STATUS:\s*(CONTINUE|CONVERGED)\s*$/i`. If none, →
   `MALFORMED`.
2. If more than one match, take the **last** one — models sometimes echo the instruction
...
4. `ISSUES:` section (only relevant when `CONTINUE`) = lines starting with `-` or `*`
   ...
   issue lines is still valid (empty array) — it is not itself malformed, though the
   orchestrator logs a warning since it's an unusual shape.
5. `CONVERGED` with a stray `ISSUES:` section present → still `CONVERGED`; the issues are
   logged into the transcript as trailing notes but do not affect stopping.

**Orchestrator handling of `MALFORMED`:** one automatic recovery attempt — send a short,
...
human to read the transcript — no further automatic retries.
...
New pure module `src/orchestrator/prompts.ts` — plain functions, no side effects, each
...
**Claude reviewer prompt — round 1** (`buildInitialClaudeReviewPrompt(task, chatgptResponse)`):
```
...
the task — do not invent minor or speculative objections merely to keep the review going.
...
**ChatGPT revision prompt — round N≥2** (`buildRevisionPrompt(issues)`):
```
...
criticized — do not restart from scratch.
...
**Claude subsequent review prompt — round N≥2**
(`buildFollowUpClaudeReviewPrompt()`):
```
Here is the revised answer, addressing your previous feedback. Review it again with the
same criteria as before. Only raise issues that are new or still genuinely unresolved — do
...
```
(No task/response text is re-pasted here — both models already have the full
...
- `transcript.md`: human-readable, append-only — one section per completed turn, rebuilt
  (not hand-diffed) from `rounds/*.md` each time a turn completes, so it's always
  consistent with the source-of-truth round files.
- `final.md`: written once, at stop time — the last ChatGPT answer plus the final
...
- `state.json`: the resumable machine state —
  ```
  {
    "runId": "...",
    "task": "...",
...
- **Atomic writes:** every `state.json` update is written to `state.json.tmp` then
  `fs.renameSync`'d over `state.json` — never a partial/corrupt state file if the process
  dies mid-write.
- **Resume (`resume(runId)`):** load `state.json`, then before trusting `phase`,
  ...
  process died after the ChatGPT turn completed but before persisting — resume treats the
...
|---|---|---|
| ChatGPT tab missing | `discoverSitePages` (unchanged, Phase 1) returns no `chatgpt` page at startup | Do not start a run; log and exit non-zero — same as Phase 1's `navigation_failed`-on-missing-tab behavior, reused as-is. |
| Claude tab missing | same, for `claude` | same |
| `login_required` (pre-flight or mid-run) | `checkReady(page)` re-run before every send (§3) | Persist `state.json` with `phase: "paused_login_required"`, print instructions to log in manually in the already-open Chrome window and re-run with `--resume <run-id>`, exit non-zero. **Never** attempts to fill/click a login form. |
...
| Malformed convergence status | `convergence.ts` returns `MALFORMED` | One automatic reformat-retry (§5); if still malformed, `phase: "error_malformed_convergence"`, exit non-zero. |
| Ambiguous new-message count | `getLatestAssistantResponse` / `resolveNewAssistantMessage` (§10) finds 0 or >1 new message ids | `phase: "error_ambiguous_response"`, exit non-zero — never guesses which message is "the" new one. |
...
`claudeSelectors.ts` — each exports **ordered arrays** of selector strings per role, tried
...
generated class names, per `SPEC.md` §16's stated mitigation for UI churn.

```
// chatgptSelectors.ts (illustrative — verified/updated at implementation time)
composer: [
...
```
```
// claudeSelectors.ts (illustrative — verified/updated at implementation time)
composer: [
...
DOM during Milestone 1/2 implementation (§14) — the point of the design is the **fallback
...
- `captureTurnBaseline` is called **before** `sendPrompt`, not after — this is the anchor
...
- `resolveNewAssistantMessage` re-queries the assistant message nodes after generation
  ...
  success path — anything else (`0` = nothing appended yet/extraction ran too early, `>1` =
  more than one message appended, e.g. a regenerate produced a duplicate) is surfaced as
  `AmbiguousNewMessageError`, handled per §8's table — **never** silently falls back to
...
All automated tests stay local/offline, extending Phase 1's pattern (`SPEC.md` §14A) —
real ChatGPT/Claude interaction remains a manual checklist
...
a small inline script for ChatGPT-flavored vs Claude-flavored attribute names) — a tiny
...
| `tests/unit/convergenceParser.spec.ts` | `CONVERGED`/`CONTINUE` happy paths, case-insensitivity/whitespace variance, multiple `STATUS:` lines (last wins), stray `ISSUES:` under `CONVERGED`, no `STATUS:` line at all → `MALFORMED`. |
| `tests/unit/promptTemplates.spec.ts` | Each template function's output against fixed inputs (snapshot-style) — mainly guards against accidental prompt-format drift. |
...
| `tests/unit/toolIsolation.spec.ts` (extends Phase 1's `openSite.spec.ts` pattern) | One site's adapter throwing/timing out does not corrupt the other site's turn or crash the orchestrator — reuses Phase 1's existing failure-isolation proof, extended to the conversational path. |

`package.json`'s `test` script and `playwright.config.ts`'s `testDir: "./tests/unit"`
need no changes — new spec files are picked up automatically.
...
`MAX_ROUNDS_REACHED` (or a safety pause per §8).
...
`shouldPauseBeforeNextRound?()` hook on `ReviewOrchestrator` (§3) — a single, optional,
...
- `src/main.ts` (Phase 1's entry point stays the Phase 1 smoke-check entry point —
...
**Minimally extended (additive only — no existing exported signature changes):**
- `src/config/loadConfig.ts` — add `maxRounds` (default `12`), `generationTimeoutMs`
...
- `src/sites/siteTypes.ts` — add a new `ConversationalSiteAdapter extends SiteAdapter`
  interface (the four new methods from §1/§2) in the same file, rather than modifying
  `SiteAdapter` itself — Phase 1's `SiteAdapter`/`checkReady`-only usage in `main.ts`/
  `openSite.ts` stays valid and untouched.
- `src/sites/chatgptSite.ts` / `src/sites/claudeSite.ts` — extended in place with the new
...
- `.gitignore` — add `runs/` (and, independent cleanup noted in `REVIEW.md`,
...
**Milestone 1 — ChatGPT send + wait + latest-response detection**
...
**Milestone 4 — Multi-round orchestrator + convergence loop**
Files: `orchestrator/types.ts`, `orchestrator/reviewOrchestrator.ts` (full state machine:
revision prompts, `maxRounds`, all §8 safety states, stalled-loop guard),
...
every pause/error path in §8's table.

**Milestone 5 — Persistence + resume**
...
full manual end-to-end run against real ChatGPT/Claude (2–3 rounds) is completed,
...
- **Recommended architecture:** two thin `ConversationalSiteAdapter` implementations
  ...
  resume. This mirrors Phase 1's own separation of concerns (`DECISIONS.md` §6) instead of
  introducing a new pattern.
- **Biggest technical risks, roughly in order:**
  1. **Selector/DOM fragility** — ChatGPT and Claude can change their markup at any time;
     mitigated by the fallback-array pattern (§9) but never eliminated, and the
     illustrative selectors in this doc are unverified starting points, not final values.
  2. **Generation-completion false signals** — a UI that clears its "stop" button before
     ...
     design in §1, but this is the single most likely source of subtle bugs.
  3. **Response confusion** (old vs. new message) — mitigated by the baseline/id-diff
     design in §10, but depends on the site consistently exposing *some* stable per-message
     identity; if it doesn't, the fallback (index+handle pairing) is weaker.
  4. **12 rounds × 2 sites of real, unattended web-UI automation** is a long unattended
     window in which a Cloudflare re-challenge, a session expiry, or a UI update can occur;
     the pause-and-resume design (§7/§8) is what keeps a mid-run failure cheap rather than
  ...
  5. Everything else (empty responses, malformed convergence, refusals) is comparatively
     easy — deterministic checks with a documented, bounded response (§8).
- **Exact first implementation milestone:** Milestone 1 — ChatGPT send + wait +
