...
- It supports **persistent browser contexts** (`launchPersistentContext`) with a
  ...
  sessions to survive between runs (see §5).
- It has first-class TypeScript support and a stable, well-documented API for navigation,
  waiting for elements, and state inspection — needed for the readiness-detection work in
...
- It is actively maintained, widely used for browser automation, and does not require any
  private/reverse-engineered protocol — it automates the browser the same way a human
...
Same reasoning as §2: the user has a normal Claude web account and does not want to incur
...
- Private endpoints are undocumented, unstable, and can change or be revoked without
  notice — this is precisely what killed the project's previous approach (see §9,
  ChatHub).
...
Logging into ChatGPT and Claude typically involves passwords, 2FA, and/or SSO — a flow
...
The persistent `userDataDir` (§8 of `SPEC.md`) **must** be a folder dedicated to this
...
`userDataDir` — the executable may be the user's real Chrome; the profile data directory
...
- **Keep automation state isolated.** A dedicated profile means the only cookies/sessions
  present are the ones this tool created for ChatGPT and Claude — nothing else. This makes
...
- **The user logs into ChatGPT and Claude once, inside the dedicated automation
  profile** — a separate, intentional login distinct from (and with no bearing on) their
  normal daily browser session. This is the login this project's §8a interactive login
  flow waits on.
...
- It is the mechanism that makes "modular support for additional AI web interfaces later"
  (an explicit long-term goal) actually achievable — adding a third AI means adding one
...
project is, what's been done, and why — not any AI chat session's memory. This is because:
...
- `TODO.md` in particular must reflect **verified, actual** implementation state (per its
  own rules), so any agent — or the user — can trust it as ground truth without needing to
...
this file — as part of the same change, not as an afterthought.**
...
- Every later phase (adapters, orchestrator, state machine) depends on the browser
  foundation being solid and well-understood first — building message-sending or
...
- This matches the project's explicit guidance to avoid overengineering and unnecessary
  frameworks — build the smallest useful slice, verify it, then extend.
...
- Its **ChatGPT web adapter** failed at runtime with a `"Failed to fetch"` error —
  consistent with it depending on ChatGPT backend endpoints/mechanisms that have since
...
Playwright against the normal rendered UI** (see §1 and §4). This is expected to be more
...
content from either site — it only detects page load/readiness state. This decision
...
Phase 2, as currently sketched in `SPEC.md`/`TODO.md`, proposes exactly this —
`extractLatestResponse(page)` reading ChatGPT's generated text out of the DOM
...
1. Re-evaluate OpenAI's *then-current* service terms specifically regarding automated/
   ...
   rely on this document's description staying accurate — verify at implementation time).
2. Choose a compliant approach before writing extraction code. Possible directions
   (none pre-selected — this is a decision for whoever starts Phase 2, informed by
